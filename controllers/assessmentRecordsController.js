...
const createAssessmentRecord = async (req, res) => {
  try {
    ...
    let recommendation = "";
    if (Math.max(categories) === 0) recommendation = `You scored ${score}/${score} on the '${module.title}' assessment! Keep up the good work!`;
    else recommendation = `You scored ${score}/${module.questions.length} on the '${module.title}' Consider studying more about ${getMaxWrongAnswers(categories, module.categories)} to help boost your score!`;
